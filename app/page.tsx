      ...
      <div className="max-w-5xl mx-auto px-5 py-10 space-y-12">
        {featured ? (
            ...
            <Link href={`/noticias/${featured.id}`} className="group block">
              <div className="rounded-2xl border border-white/10 group-hover:border-accent/40 transition-all overflow-hidden">
                <div className="p-8 sm:p-10">
                  ...
                  <h1 className="font-display text-3xl sm:text-4xl text-bone leading-tight mb-4 group-hover:text-accent transition-colors">
                    {featured.titulo}
                  </h1>
                  {featured.resumo && (
          ...
          <section className="rounded-2xl border border-white/5 p-10 text-center">
            <p className="font-display text-3xl text-bone/20">Em breve</p>
        ...
        {rest.length > 0 && (
          <section>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {rest.map((p) => (
                <Link key={p.id} href={`/noticias/${p.id}`} className="group rounded-xl border border-white/8 hover:border-accent/30 p-5 transition-all flex flex-col gap-3">
                  <span className="text-[10px] text-bone/30">{formatDate(p.data_pub)}</span>
                  <h2 className="font-display text-lg text-bone group-hover:text-accent transition-colors leading-snug line-clamp-3">{p.titulo}</h2>
                  {p.resumo && <p className="text-bone/40 text-sm leading-relaxed line-clamp-3 flex-1">{p.resumo}</p>}
                  <span className="text-xs text-accent/60 group-hover:text-accent transition-colors">Ler →</span>
                </Link>
              ))}
            </div>
          </section>
